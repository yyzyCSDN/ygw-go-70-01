...
func (c *Client) Patrol(invIDs []string) error {
	for _, invID := range invIDs {
		...
		s, err := c.OpenSession(invID)
		if err != nil {
			_ = c.table.ReleaseLease(invID, leaseID)
			continue
		}
		...
		if err := s.handle.Write(EncodeFrame(frame)); err != nil {
			_ = c.CloseSession(invID)
			_ = c.table.ReleaseLease(invID, leaseID)
			continue
		}
		if _, err := s.handle.Read(); err != nil {
			_ = c.CloseSession(invID)
			_ = c.table.ReleaseLease(invID, leaseID)
			continue
		}
...
type Hub struct {
	...
	droppedFrames [][]byte
	lastFrame    *inverter.Frame
}

func NewHub() *Hub {
	return &Hub{clients: make(map[string]*Client), droppedFrames: make([][]byte, 0)}
}

...
type Table struct {
	...
	modelHistory   map[string][]string
	leaseSeq       int
	now            func() time.Time
}
...
func NewTable() *Table {
	return &Table{
		...
		replacementLog: make(map[string]string),
		modelHistory:   make(map[string][]string),
		now:            time.Now,
	}
}
...
func (t *Table) Replace(id, serial, model string) error {
	...
	t.modelHistory[id] = append(t.modelHistory[id], inv.Model)
	inv.Serial = serial
	inv.Model = model
	inv.ModelRev++
	inv.ModelChangedAt = t.now()
	t.replacementLog[id] = model
	return nil
}
...
func (t *Table) MessageStatusesFor(batch []QueuedMessage) map[int]MessageResult {
	...
	out := make(map[int]MessageResult, len(inv.Messages))
	for seq, res := range inv.Messages {
		out[seq] = res
	}
	for seq, res := range out {
